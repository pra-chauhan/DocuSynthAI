...
def get_risk_suggestion(sentence: str, category: str) -> str:
    ...
    try:
        if llm is None:
            return "⚠️ LLM not initialized. Check GROQ_API_KEY and rag.load_llm()"
        ...
        return f"⚠️ Failed to generate suggestion: {e}"
    ...
    if st.session_state.document_processed:
        with tab2:
            ...
            with st.container(border=True):
                ...
                risk_data = st.session_state.get("risk_data", {})  # ✅ ensure it's defined

                if risk_data.get('categories'):
                    df = pd.DataFrame.from_dict(risk_data['categories'], orient='index')

                    # Drop examples for the dataframe view (keeps table clean)
                    if "examples" in df.columns:
                        df_table = df.drop(columns=["examples"])
                    else:
                        df_table = df
                    ...
                    # Expanders for examples
                    st.subheader("📌 Risky Sentences per Category")
                        ...
                        with st.expander(f"{category} Risks ({details['count']})"):
                            ...
                            if details.get("examples"):
                                ...
                                for i, sentence in enumerate(details["examples"], 1):
                                    ...
                                    for keyword in risk_categories[category]['keywords']:
                                        pattern = r"\b" + re.escape(keyword) + r"\b"
                                        # replace matched keyword (preserve original case)
                                        def repl(m):
                                            return f"<span style='color:#dc3545; font-weight:600'>{m.group(0)}</span>"
                                        highlighted_sentence = re.sub(pattern, repl, highlighted_sentence, flags=re.IGNORECASE)

                                    # display the sentence (allow HTML)
                                    st.markdown(f"- {highlighted_sentence}", unsafe_allow_html=True)

                                    # create a safe button key (no spaces)
                                    cat_key = category.replace(" ", "_")
                                    btn_key = f"suggest_{cat_key}_{i}"

                                    # Generate suggestion on-demand and persist it in session_state
                                    if st.button(f"💡 Suggest Fix", key=btn_key):
                                        with st.spinner("Generating suggestion..."):
                                            suggestion = get_risk_suggestion(sentence, category)
                                            # persist so suggestion remains visible after reruns
                                            st.session_state[f"suggestion_{cat_key}_{i}"] = suggestion

                                    # if we already have a persisted suggestion for this sentence, show it
                                    saved_key = f"suggestion_{cat_key}_{i}"
                                    if st.session_state.get(saved_key):
                                        st.info(st.session_state[saved_key])
